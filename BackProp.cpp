...
//  saves state
void CBackProp::save(char* filename)
{
    ...
    // layer structure
    for (int i=0; i<numl; ++i)
        fprintf(file, ", %i", lsize[i]);
    fprintf(file, "\n");
    // learning rate (beta) and momentum (alpha)
    fprintf(file, "%e, %e\n\n", beta, alpha);
    ...
    for(int i=1; i<numl; ++i)
    {
	    for(int j=0;j<lsize[i];j++)
        {
		    for(int k=0; k<(lsize[i-1]+1); ++k)
            {
                if (k > 0)
                    fprintf(file, ", ");
			    fprintf(file, "%+1.16f", weight[i][j][k]);
            }
            fprintf(file, "\n");
        }
        fprintf(file, "\n");
...
//  restores state
void CBackProp::restore(char* filename)
{
    ...
    // layer structure
    for (int i=0; i<numl; ++i)
        fscanf(file, ", %i", &lsize[i]);
    // learning rate (beta) and momentum (alpha)
    int a = fscanf(file, "%e, %e", &beta, &alpha);
    ...
    for(int i=1; i<numl; ++i)
    {
	    for(int j=0;j<lsize[i];j++)
        {
            ...
            {
                if (k == 0)
                    int a = fscanf(file, "%lf", &weight[i][j][k]);
                else
                    int a = fscanf(file, ", %lf", &weight[i][j][k]);
            }
        }
